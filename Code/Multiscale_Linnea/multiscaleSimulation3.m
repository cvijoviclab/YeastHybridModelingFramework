...
%add path to functions and results
addpath ('sourceCode');
[path] = createResultsFolder('multiscale', yyyymmdd(datetime));
...
settings.enzymeUse = 0.003; %put in value from the analysis
...
fluxDist.start      = zeros(size(fluxDist,1),1);
Enz_usages          = table();
...
RxnIdx  = [7 45 60 61 62 63 64 69 158];
%Simulations!
for subopt_growth = 0:(0.4/80):0.4
    ...
    %Change Boolean
    [Bool,settings] = connectFBA2Bool(Bool,settings,fluxDist(RxnIdx,:),(i+4));
    [Bool, TFAct, EnzAct] = runBool(Bool,settings);
    [GeneExpr] = expression(TFAct.End, tempModel); % values are between -4 and 4 if deviding into differnt levels later. 
...
writetable(Enz_massUsages,[path,'enzMassUsages_reducedYeast.txt'],'Delimiter','\t','QuoteStrings',false);
...
figure()
plot(0:(0.4/80):0.4,table2array(fluxDist(RxnIdx,7:87))','LineWidth',3)
legend(table2array(fluxDist(RxnIdx,2)))
xlabel('Dilution rate [1/h]','FontSize',18)
ylabel('Fluxes [mmol/gDw h]','FontSize',18)
writetable(fluxDist(RxnIdx,:),[path,'booleanInput.txt'],'Delimiter','\t','QuoteStrings',false);