%% Script to define structure of boolean model; Bool
%written by: Julia Münch and Linnea Österberg
%date: 2019-10-31 - 2020-04-02
%description: define the model structure of the Boolean model including 
%pathways and respective components and set the initial condition for a 
%Boolean Network including
%   (1) Metabolites
%   (2) Target Genes
...
%   (5) Snf1 pathway
%   (6) TOR pathway
%creates struct with matrix representation of each pathway 
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Name = [string('PFK2'), string('NTH1'), string('CDC19'), string('FBP1'), string('ACC'), string('HXK2')]';
presence = ones(length(Name), 1);
phosphorylation = zeros(length(Name),1);
Enzymes = table(Name, presence, phosphorylation);
Enzymes{6,3} = 1;
Bool.Enzymes=Enzymes;
%%% PKApw %%%
