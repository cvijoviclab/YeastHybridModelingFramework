function[rank] = getRanks2(TFAct)

for i=2:width(TFAct)
    TFAct.(i) = str2double(TFAct{:,i});
end
%enzNames = model.enzNames;
enzNames = table2array(readtable('../Combined/POI.txt', 'ReadVariableNames', false));
rank = zeros(length(enzNames),1);
rank = table(enzNames, rank);
var = size(TFAct, 2);

expression = '\,';
TF = readtable('../Combined/TF-Targets-import.xlsx');
...
if TFAct{1,var} == 1
    ...
    for i = 1:length(down)
        rank{down(i),2} = rank{down(i),2} - 0.5;
    end
end
if TFAct{1,var} == 0
    down = find(ismember(enzNames, GisDown));
