function [Metabolites, PKApw, Snf1pw, TORpw, Enzymes, Targets, TFActStart, TFActEnd, EnzActStart, EnzActEnd] = ...
    reachSteadyState(MetabolitesIn, PKApwIn, Snf1pwIn, TORpwIn, EnzymesIn, TargetsIn, gluc, nitr, path, activeCrosstalk)
...
Metabolites{1,2} = gluc; %determine glucose concentration (0) no, (1) high
Metabolites{5,2} = nitr;
...
while true
    ...
    if iteration == 1
        ...
        [TFActStart] = TFtargets(PKApwAct, Snf1pwAct, TORpwAct);
        [EnzActStart] = EnzymesAct;
     end
    ...
    %AC is activated when RAS is activated, activation through GPA2 requires presence of phosphorylated glucose (here: fructose-1,6-bp as placeholder)    %by active Ras (Conrad et al., 2014) 
    if PKApwOld{5,4} == 1 && PKApwOld{7,2} == 1 || (Metabolites{4,2} == 1 && PKApwOld{2,4} == 1 && PKApwOld{7,2} == 1)
        PKApw{7,4} = 1;
    end
    if PKApwOld{7,2} == 0
        PKApw{7,4} = 0; 
    ...
    %PKA is active when cAMP is there and Krh is inactive or if Bcy not
    %there
    if (PKApwOld{13,2} == 1 && MetabolitesOld{3,2} == 1 && PKApwOld{3,4} == 0 && PKApw{12,2} == 1) || PKApwOld{12,2} == 0
        PKApw{13,4} = 1;
    end
    ...
    %active PKA phosphorylates TREH, PFK2/F-2,6-BP, PK, F-1,6-BP and Rim15
    ...
    if PKApwOld{13,4} == 1 && EnzymesOld{7,2} == 1 
        Enzymes{7,3} = 1;
    else
        Enzymes{7,3} = 0;
    end
    
    %PFK2/FBP2 is bifunctional enzyme -> acts as FBP2 if phosphorylated and as PFK2 if unphosphorylated 
...
%create file with transitions of TF factors
[TFActEnd] = TFtargets(PKApwAct, Snf1pwAct, TORpwAct);
[EnzActEnd] = EnzymesAct;
