function [Mutant] = BoolToFBA2(ExpressionChange, EnzActChange, model_in, perc)
...
%% make Mutants!
for i=1:height(EnzActChange)
    if contains(EnzActChange{i,1}, ExpressionChange{:,1})
        idx=find(contains(ExpressionChange{:,1}, EnzActChange{i,1}));
        if EnzActChange{i,2} > 0
            ExpressionChange{idx,2} = ExpressionChange{idx,2} +2;
        elseif EnzActChange{i,2} < 0
            ExpressionChange{idx,2} = ExpressionChange{idx,2} -2;
        end
    end
end
GeneID = ExpressionChange{(ExpressionChange{:,2} ~= 0), 3};
sel_ExpressionChanges = ExpressionChange{(ExpressionChange{:,2} ~= 0), 2};
