...
#reaction_df_selec %>% filter(grepl("MIG1",input)) -> reaction_df_selec_MIG1_input
#reaction_df_selec %>% filter(grepl("MIG1",output) | grepl("MIG1",input)) -> reaction_df_selec_MIG1_both


text_file <- read_csv("POI.txt", col_names = F, col_types = cols())
reaction_df_selec_tibble <- as_tibble(reaction_df_selec) %>%
  mutate(output = as.character(output))
dir_save <- "Sorted_data/"
if(!dir.exists(dir_save)) dir.create(dir_save)
for(i in 1:dim(text_file)[1]){
  file_name <- str_c(dir_save, text_file$X1[i], ".csv")
  data_save <- reaction_df_selec_tibble %>%
    filter(grepl(text_file$X1[i], output))
  write_csv(data_save, file_name)
}

text_file <- read_csv("TF_list.txt", col_names = F, col_types = cols())
dir_save <- "Sorted_data_TFbinding/"
if(!dir.exists(dir_save)) dir.create(dir_save)
for(i in 1:dim(text_file)[1]){
  file_name <- str_c(dir_save, text_file$X1[i], ".csv")
  data_save <- reaction_df_selec_tibble %>%
